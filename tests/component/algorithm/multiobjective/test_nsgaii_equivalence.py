"""Acceptance test for Phase 1: build_nsgaii() must match the classic NSGAII.

Both implementations reuse the exact same operator classes (SBXCrossover,
PolynomialMutation, TournamentSelection, RankingAndDensityEstimatorReplacement,
FastNonDominatedRanking, CrowdingDistanceDensityEstimator) and the exact same
Problem.create_solution() -- so, given identical random state, they must draw the
same random numbers in the same order and therefore produce identical fronts. Two
sources of randomness are involved: Python's global `random` module (used by
Problem.create_solution() and TournamentSelection) and each crossover/mutation
operator's own `np.random.Generator` (used by SBXCrossover and PolynomialMutation).
"""
...
class TestBuildNSGAIIMatchesTheClassicNSGAII:
