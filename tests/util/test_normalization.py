...
class TestNormalizationUtilities(TestCase):
    ...
    def test_normalize_to_unit_hypercube(self):
        ...
        np.testing.assert_allclose(
            np.min(normalized, axis=0), [0.0, 0.0], atol=NORMALIZATION_TEST_ATOL
        )
        np.testing.assert_allclose(
            np.max(normalized, axis=0), [1.0, 1.0], atol=NORMALIZATION_TEST_ATOL
        )
    ...
    def test_identical_fronts(self):
        ...
        norm_front, norm_ref = normalize_fronts(
            identical_front, identical_front, method="reference_only"
        )
    ...
    def test_constant_column_in_reference(self):
        ...
        np.testing.assert_allclose(
            norm_front[:, 0], expected_front_obj1, atol=NORMALIZATION_TEST_ATOL
        )
    ...
    def test_solutions_to_matrix_with_mock_solutions(self):
        ...
        solutions = [MockSolution([1.0, 2.0]), MockSolution([3.0, 4.0]), MockSolution([5.0, 6.0])]
    ...
    def test_mathematical_properties_preservation(self):
        ...
        np.testing.assert_allclose(
            normalized_diff_obj1_12, normalized_diff_obj1_23, atol=NORMALIZATION_TEST_ATOL
        )
