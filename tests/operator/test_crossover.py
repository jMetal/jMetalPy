...
class TestSinglePointCrossover:
    ...
    @pytest.mark.parametrize(
        "probability,expected_msg",
        [
            (-0.5, "Probability must be between 0.0 and 1.0"),
            (1.5, "Probability must be between 0.0 and 1.0"),
        ],
    )
    def test_should_constructor_raise_exception_for_invalid_probability(
        self, probability, expected_msg
    ):
...
class TestSBXCrossover:
    ...
    @pytest.mark.parametrize(
        "probability,distribution_index",
        [
            (0.1, 2.0),  # Valid case
    ...
    def test_should_execute_raise_exception_for_three_solutions(self):
        ...
        with pytest.raises(InvalidConditionException, match="The number of parents is not two: 3"):
            crossover.execute(
                [FloatSolution([0], [1], 1), FloatSolution([0], [1], 1), FloatSolution([0], [1], 1)]
            )
    ...
    def test_should_work_with_solution_subclass(self):
        ...
        class CustomSolution(FloatSolution):
            def __init__(
                self, lower_bound, upper_bound, number_of_objectives, number_of_constraints=0
            ):
                super().__init__(
        ...
        for i in range(len(offspring[0].variables)):
            assert (
                offspring[0].lower_bound[i]
                <= offspring[0].variables[i]
                <= offspring[0].upper_bound[i]
            )
            assert (
                offspring[1].lower_bound[i]
                <= offspring[1].variables[i]
                <= offspring[1].upper_bound[i]
            )
...
class TestPMXCrossover:
    ...
    def test_should_work_with_permutation_at_middle(self, mock_sample):
        ...
        for child in offspring:
            assert sorted(child.variables) == list(range(10)), (
                "Offspring is not a valid permutation"
            )
    ...
    def test_should_work_with_permutation_at_beginning(self, mock_sample):
        ...
        assert len(set(beginning_segment)) == len(beginning_segment), (
            "Duplicate values in beginning segment"
        )
...
class TestCXCrossover:
    ...
    def test_should_work_with_probability_zero(self, mock_randint):
        ...
        assert (
            offspring[0].variables == solution1.variables
            or offspring[0].variables == solution2.variables
            or offspring[1].variables == solution1.variables
...
class TestBLXAlphaBetaCrossover:
    ...
    @pytest.mark.parametrize(
        "probability,alpha,beta,expected_alpha,expected_beta",
        [
    ...
    def test_should_constructor_assign_correct_values(
        self, probability, alpha, beta, expected_alpha, expected_beta
    ):
...
class TestBLXAlphaCrossover:
    ...
    @pytest.mark.parametrize(
        "probability,alpha,expected_alpha",
        [
