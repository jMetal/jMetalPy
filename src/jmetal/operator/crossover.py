...
from jmetal.operator.repair import ClampFloatRepair, FloatRepairOperator, ensure_float_repair
...
class SBXCrossover(Crossover[FloatSolution, FloatSolution]):
    ...
    def execute(self, parents: list[FloatSolution]) -> list[FloatSolution]:
        ...
        Check.that(len(parents) == 2, f"The number of parents is not two: {len(parents)}")
                    ...
                    if abs(value_x1 - value_x2) > self.__EPS:
                        ...
                        try:
                            ...
                            if rand_val <= (1.0 / alpha1):
                                betaq1 = pow(
                                    rand_val * alpha1, (1.0 / (self.distribution_index + 1.0))
                                )
                            else:
                                betaq1 = pow(
                                    1.0 / (2.0 - rand_val * alpha1),
                            ...
                            if rand_val <= (1.0 / alpha2):
                                betaq2 = pow(
                                    (rand_val * alpha2), (1.0 / (self.distribution_index + 1.0))
                                )
                            else:
                                betaq2 = pow(
                                    1.0 / (2.0 - rand_val * alpha2),
                                    1.0 / (self.distribution_index + 1.0),
                                )
...
class CompositeCrossover(Crossover[CompositeSolution, CompositeSolution]):
    ...
    def __init__(self, crossover_operator_list: list[Crossover]):
        ...
        for operator in crossover_operator_list:
            Check.that(
                issubclass(operator.__class__, Crossover), "Object is not a subclass of Crossover"
            )
