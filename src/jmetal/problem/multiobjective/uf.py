...
class UF1(FloatProblem):
    ...
    def evaluate(self, solution: FloatSolution) -> FloatSolution:
        ...
        solution.objectives[1] = (
            1.0 - sqrt(x[0]) + 2.0 * sum2 / count2 if count2 > 0 else 1.0 - sqrt(x[0])
        )
    ...
    def evaluate(self, solution: FloatSolution) -> FloatSolution:
        ...
        for i in range(2, n_vars):
            y_base = (
                0.3
                * x[0]
                * (x[0] * np.cos(24.0 * np.pi * x[0] + 4.0 * (i + 1) * np.pi / n_vars) + 2.0)
                * np.sin(6.0 * np.pi * x[0] + (i + 1) * np.pi / n_vars)
            )
            y = x[i] - y_base
            if (i + 1) % 2 == 1:  # odd indices (3, 5, 7, ...)
                sum1 += y * y
...
class UF3(FloatProblem):
    ...
    def evaluate(self, solution: FloatSolution) -> FloatSolution:
        ...
        solution.objectives[0] = x[0] + 2.0 * (4.0 * sum1 - 2.0 * prod1 + 2.0) / np.floor(
            n_vars / 2.0
        )
        solution.objectives[1] = (
            1.0
            - np.sqrt(x[0])
            + 2.0 * (4.0 * sum2 - 2.0 * prod2 + 2.0) / np.ceil(n_vars / 2.0 - 1.0)
        )
...
class UF7(FloatProblem):
    ...
    def evaluate(self, solution: FloatSolution) -> FloatSolution:
        ...
        solution.objectives[1] = (
            1.0 - np.power(x[0], 0.2) + 2.0 * sum2 / np.ceil(n_vars / 2.0 - 1.0)
        )
...
class UF8(FloatProblem):
    ...
    def evaluate(self, solution: FloatSolution) -> FloatSolution:
        ...
        solution.objectives[0] = (
            np.cos(0.5 * np.pi * x[0]) * np.cos(0.5 * np.pi * x[1]) + 2.0 * sum1 / count1
        )
        solution.objectives[1] = (
            np.cos(0.5 * np.pi * x[0]) * np.sin(0.5 * np.pi * x[1]) + 2.0 * sum2 / count2
        )
