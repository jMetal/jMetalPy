...
def _f6(y: np.ndarray, n_obj: int) -> np.ndarray:
    ...
    numerator = (
        (1.0 + math.exp(2.0 * k * mu - k)) ** -1.0 - r * mu - (1.0 + math.exp(k)) ** -1.0 + r
    )
...
class ZCAT(FloatProblem):
    def __init__(
        self,
        problem_id: int,
        number_of_variables: int = 30,
        ...
        if number_of_variables <= 0:
            raise ValueError("ZCAT requires number_of_variables > 0.")

        min_required_variables = (
            1 if problem_id in _ONE_DIMENSIONAL_PARETO_SET_PROBLEMS else number_of_objectives - 1
        )
        if number_of_variables < min_required_variables:
            raise ValueError(
                f"ZCAT{problem_id} requires number_of_variables >= {min_required_variables} for "
                f"number_of_objectives={number_of_objectives}. Received number_of_variables={number_of_variables}."
            )
        ...
        indices = np.arange(1.0, number_of_variables + 1.0, dtype=float)
        self.lower_bound = (-0.5 * indices).tolist()
        self.upper_bound = (0.5 * indices).tolist()
    ...
    def _pareto_set_dimension(self, y0: float) -> int:
        ...
        if self.problem_id == 19:
            if _value_in(y0, 0.0, 0.2) or _value_in(y0, 0.4, 0.6):
                return 1
            return self.number_of_objectives() - 1
    ...
    def evaluate(self, solution: FloatSolution) -> FloatSolution:
        ...
        if x.size != self.number_of_variables():
            raise ValueError(
                f"Expected {self.number_of_variables()} decision variables, got {x.size}."
            )
        ...
        alpha = _evaluate_f(self.problem_id, y, self.number_of_objectives())
        for objective_index in range(1, self.number_of_objectives() + 1):
            alpha[objective_index - 1] = (objective_index**2.0) * float(alpha[objective_index - 1])

        beta = np.zeros(self.number_of_objectives(), dtype=float)

        if pareto_set_dimension != self.number_of_variables():
            g_values = _evaluate_g(
                self._g_function_id(), y, pareto_set_dimension, self.number_of_variables()
            )
            ...
            w_size = self.number_of_variables() - pareto_set_dimension
            for objective_index in range(1, self.number_of_objectives() + 1):
                j_values = _get_j(objective_index, self.number_of_objectives(), w_values, w_size)
                z_score = _evaluate_z(j_values, objective_index, self.imbalance, self.level)
                beta[objective_index - 1] = (objective_index**2.0) * z_score

        f_values = alpha + beta
        solution.objectives = [float(value) for value in f_values]
        return solution

    def name(self) -> str:
        return f"ZCAT{self.problem_id}"
