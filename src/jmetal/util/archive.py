...
class VectorizedNonDominatedSolutionsArchive(Archive[S]):
                ...
                if dominance_flag == 1:
                    ...
                    if self._objectives is None:
                        if self._objectives is None:
                            ...
                            if self._objectives is None and len(solution.objectives) == len(
                                current_solution.objectives
                            ):
                                if all(
                                    abs(a - b) <= self.objective_tolerance
                                    for a, b in zip(
                                        solution.objectives, current_solution.objectives
                                    )
                                ):
                                    return False
                    # keep current
        ...
        for current_solution in self.solution_list:
            ...
            if dominance_flag == 1:
                ...
                if self._objectives is not None:
                    ...
                    if np.all(
                        np.abs(self._objectives[idx] - np.asarray(solution.objectives, dtype=float))
                    ...
                    if all(
                        abs(a - b) <= self.objective_tolerance
                        for a, b in zip(solution.objectives, current_solution.objectives)
                    ):
                        return False
                remaining_solutions.append(current_solution)
    ...
    def _add_two_objective(self, solution: S) -> bool:
        ...
        kept_obj0s = (
            np.array([s.objectives[0] for s in new_solutions]) if new_solutions else np.array([])
        )
        pos = (
            int(np.searchsorted(kept_obj0s, new_obj[0], side="right")) if kept_obj0s.size > 0 else 0
        )
...
def distance_based_subset_selection_robust(
    ...
    return _robust_distance_based_selection(
        solution_list, subset_size, metric, weights, use_vectorized, rng
    )
    ...
    if metric == DistanceMetric.TCHEBY_WEIGHTED:
        if weights is None:
            ...
            if len(weights) != n_objectives:
                raise ValueError(
                    f"Weights length ({len(weights)}) must match number of objectives ({n_objectives})"
                )
...
def _original_subset_selection(
    solution_list: list[S],
    ...
    return _vectorized_subset_selection(
        solution_list, normalized_matrix, subset_size, seed_idx, metric, weights, rng
    )


def _vectorized_subset_selection(
    solution_list: list[S],
    normalized_matrix: np.ndarray,
    subset_size: int,
    seed_idx: int,
    ...
    if metric == DistanceMetric.L2_SQUARED:
        return _vectorized_subset_selection_l2(
            solution_list, normalized_matrix, subset_size, seed_idx
        )
...
def _vectorized_subset_selection_l2(
    solution_list: list[S],
    normalized_matrix: np.ndarray,
    subset_size: int,
    seed_idx: int,
