...
def _parse_ref_point(ref_point_str: str, expected_dim: int) -> list[float]:
    ...
    if len(values) != expected_dim:
        raise ValueError(
            f"Reference point dimension mismatch: expected {expected_dim}, got {len(values)}"
        )
...
def _normalize_fronts(
    front: np.ndarray, reference: np.ndarray, method: str = "reference_only"
...
def _compute_all_indicators(
    front: np.ndarray, reference: np.ndarray, ref_point: list[float] | None, margin: float = 0.1
) -> dict[str, float]:
    ...
    if ref_point is not None:
        ...
        nhv_indicator = NormalizedHyperVolume(
            reference_front=reference, reference_point_offset=margin
        )
...
def main():
    ...
    try:
        ...
        if front.shape[1] != reference.shape[1]:
            raise ValueError(
                f"Objective dimension mismatch: front has {front.shape[1]} objectives, "
        ...
        if args.indicator == "all":
            ...
            if ref_point is not None:
                ...
                indicator = NormalizedHyperVolume(
                    reference_front=reference, reference_point_offset=args.margin
                )
