...
class DistanceCalculator:
    ...
    @staticmethod
    def _tcheby_weighted(
        point1: numpy.ndarray, point2: numpy.ndarray, weights: numpy.ndarray
    ) -> float:
        """
    ...
    def _l2_squared_matrix(points: numpy.ndarray) -> numpy.ndarray:
        ...
        # ||a-b||² = ||a||² + ||b||² - 2⟨a,b⟩
        distance_matrix = (
            squared_norms[:, numpy.newaxis] + squared_norms[numpy.newaxis, :] - 2 * dot_products
        )
    ...
    @staticmethod
    def _min_l2_squared_distances(
        points: numpy.ndarray, selected_points: numpy.ndarray
    ) -> numpy.ndarray:
        ...
        distances = (
            points_norms[:, numpy.newaxis] + selected_norms[numpy.newaxis, :] - 2 * dot_products
        )
    ...
    def _min_linf_distances(points: numpy.ndarray, selected_points: numpy.ndarray) -> numpy.ndarray:
        ...
        distances = numpy.max(
            numpy.abs(points_expanded - selected_expanded), axis=2
        )  # (n_points, n_selected)
    ...
    @staticmethod
    def _min_tcheby_weighted_distances(
        points: numpy.ndarray, selected_points: numpy.ndarray, weights: numpy.ndarray
    ) -> numpy.ndarray:
...
if _NUMBA_AVAILABLE:
    ...
    def _min_l2_squared_distances_numba(points, selected_points):
        ...
        for i in range(n_points):
            minv = 1.7976931348623157e308  # large float
