...
The Quality Indicator CLI is a command-line interface for computing quality indicators between two
fronts (solution front and reference front). This tool provides an easy way to evaluate the
performance of multi-objective optimization algorithms.
...
This provides a comprehensive evaluation of your algorithm's performance compared to the reference
front.
...
- **IGD formula**: \(IGD = (\Sigma(d^{pow}))^{1/pow} / N\) where `d` is the minimum distance from each reference point to the solution front
...
This ensures that results are directly comparable with other tools in the multi-objective
